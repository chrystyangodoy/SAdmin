...
class aEvt_Curso_Participante extends mEvt_Curso_Participante {
    ...
    protected $sqlInsert = "INSERT INTO evt_curso_participante (ID_EVT_Curso_Participante, Descricao,ID_Curso,ID_Participante) VALUES ('%s','%s','%s','%s')";
    protected $sqlUpdate = "update evt_curso_participante set Descricao = '%s', ID_Curso = '%s', ID_Participante = '%s' where ID_EVT_Curso_Participante = '%s'";
    ...
    public function insert() {
        $sql = sprintf($this->sqlInsert, $this->getID_EVT_Curso_Participante(), $this->getDescricao(),$this->getID_Curso(), $this->getID_Participante());
        return $this->RunQuery($sql);
    }

    public function update() {
        $sql = sprintf($this->sqlUpdate, $this->getDescricao(),$this->getID_Curso(), $this->getID_Participante(), $this->getID_EVT_Curso_Participante());
        return $this->RunQuery($sql);
    }
